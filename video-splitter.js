...
function readableOverlayTitle(title) {
...
function centerTextFontSize(text) {
...
async function downloadYoutubeVideo(youtubeUrl, outputPath, onProgress) {
...
async function splitPart(inputPath, outputPath, startSeconds, durationSeconds, title, part, totalParts, watermarkText, onProgress) {
    ...
    if (hasLogo) {
        ...
        ffmpegArgs.push('-filter_complex', filterComplex, '-map', '[v_out]', '-map', '0:a?');
    }
    else {
    ...
    ffmpegArgs.push('-c:v', 'libx264', '-preset', 'fast', '-crf', '23', '-c:a', 'aac', '-b:a', '128k', '-movflags', '+faststart', '-progress', 'pipe:2', '-y', outputPath);
    fs.mkdirSync(path.dirname(outputPath), { recursive: true });
 ...
 */
export async function splitAndProcessVideo(options) {
    ...
    try {
        ...
        for (let index = 0; index < totalParts; index += 1) {
            ...
            await splitPart(downloadedPath, outputPath, startSeconds, partDuration, metadata.title, part, totalParts, options.watermarkText || 'TikTok Automation', options.onProgress);
