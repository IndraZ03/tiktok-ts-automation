...
export async function postTikTokAffiliateVideoApi(options) {
    ...
    const log = (msg) => console.log(`[TIKTOK_API_POST] ${msg}`);
    ...
    if (options.videoPath && fs.existsSync(options.videoPath)) {
        ...
        try {
            ...
            for (let i = 0; i < 30; i++) {
                ...
                if (i % 3 === 0) {
                    try {
                        const pageVid = await page.evaluate(`(() => {
              try {
    ...
    if (!ticketGuardPublicKey) {
        ...
        if (!ticketGuardPublicKey) {
            try {
                const dbKey = await page.evaluate(`(async () => {
          try {
    ...
    const response = await page.evaluate(evalScript);
    if (browser) {
