// grok_api_client.js - Client API Grok Imagine (State JSON, headless)
// API REST baru: Upload gambar -> POST /rest/app-chat/conversations/new (stream) -> unduh video.
...
export async function checkGrokQuota(stateName = 'indra') {
  ...
  const session = await page.evaluate(async () => (await fetch('https://grok.com/api/auth/session')).json());
  await browser.close();
...
export async function generateGrokVideoV2(options, onProgress) {
  ...
  const statePath = path.join(process.cwd(), 'grok-states', `grok-state-${stateName}.json`);
  if (!fs.existsSync(statePath)) throw new Error(`File state grok-state-${stateName}.json tidak ditemukan`);
  const log = (msg, pct = 0) => { console.log(`[GROK_V2_NEW] ${msg}`); if (onProgress) onProgress(msg, pct); };
  ...
  const resolution = options.resolution === '1080p' ? '1080p' : options.resolution === '480p' ? '480p' : '720p';
  ...
  try {
    ...
    const genPromise = page.evaluate(async (o) => {
      ...
      const handleLine = (line) => {
        ...
        let o2 = null; try { o2 = JSON.parse(line); } catch { return; }
        ...
        if (!STATE.videoUrl && STATE.conversationId) {
          ...
          for (let k = 0; k < 40; k++) {
            ...
            try {
              ...
              let jj = null; try { jj = JSON.parse(tt); } catch {}
    ...
    if (result && result.rateLimited) {
      const availableAt = result.availableAt || (rateLimitDetectedAt && rateLimitDetectedAt.availableAt) || null;
    ...
    if (!result || result.status !== 'done' || !result.videoUrl) {
      throw new Error((result && result.error) || 'Generasi video gagal di Grok (API baru).');
    }
    ...
    try {
      ...
      if (dr && dr.ok && dr.data) { fs.writeFileSync(savePath, Buffer.from(dr.data.split(',')[1], 'base64')); saved = true; }
    } catch (e) { log('Peringatan mengunduh: ' + e.message); }
...
if (process.argv[1] && process.argv[1].includes('grok_api_client')) {
