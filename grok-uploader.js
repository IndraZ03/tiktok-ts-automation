...
import { closeGrokV2Session, createGrokV2Session, generateGrokVideoV2, RateLimitError, TooManyRequestsError } from './grok_api_client.js';
...
export async function runGrokGenerator(config, log, baseDir) {
    ...
    const workers = perBrowser.map((count, idx) => runApiWorker(idx, count, config, stateDownloadDir, bahanFolderPath, promptFilePath, log));
...
//  PER-BROWSER WORKER
// ════════════════════════════════════════════════════════════
async function runApiWorker(idx, count, config, downloadDir, bahanFolderPath, promptFilePath, log) {
    const tag = `[B${idx}]`;
    ...
    try {
        ...
        for (let i = 0; i < count && isRunning; i++) {
            ...
            if (imagePath)
                log(`${tag} Gambar referensi: ${path.basename(imagePath)}`);
            try {
