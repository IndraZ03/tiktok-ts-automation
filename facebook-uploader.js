...
function formatFacebookDate(targetDateStr, defaultDateVal) {
...
async function uploadSingleFacebookVideo(page, videoPath, config, scheduleDate, scheduleTime, log, videoIndex) {
    ...
    try {
        ...
        try {
            ...
            await scheduleBtn.evaluate((el) => el.click());
...
export async function runFacebookUpload(config, log, onVideoUploaded) {
    isRunning = true;
    // Validate state file
    ...
    try {
        ...
        for (const videoFile of videosToUpload) {
            ...
            try {
                const success = await uploadSingleFacebookVideo(page, videoPath, config, schedDate, schedTime, log, uploadIndex + 1);
                if (success) {
                    successCount++;
                    if (onVideoUploaded) {
