...
function saveYtbotData(data) {
    fs.writeFileSync(YTBOT_DATA_FILE, JSON.stringify(data, null, 2));
}
function getYtbotStateVideoDir(stateFile) {
...
function getFbbotStateVideoDir(stateFile) {
...
async function extractTokopediaProductName(url) {
    const browser = await chromium.launch({
        headless: true,
...
app.post('/api/namaproduk', async (req, res) => {
...
app.get('/api/namaproduk', async (req, res) => {
...
app.post('/api/merge', mergeUpload.fields([
    ...
    const saveFolder = req.body?.saveFolder;
    ...
    try {
        const result = await mergeVideosCopyWithOptionalAudio(videoFiles.map(file => file.path), outputPath, soundFile?.path, { tempDir: path.join(__dirname, '_tmp_uploads') });
        res.json({
            success: true,
...
app.post('/api/merge/permutation', mergeUpload.fields([
    ...
    for (let idx = 0; idx < combinations.length; idx++) {
        ...
        try {
            await mergeVideosCopyWithOptionalAudio([v1Path, v2Path], outputPath, soundFile?.path, { tempDir: path.join(__dirname, '_tmp_uploads') });
            successCount++;
        }
        catch (err) {
            failCount++;
...
function saveMergeUploadConfig(config) {
...
app.get('/api/merge-upload/scan-subfolders', (req, res) => {
    ...
    if (!parent)
        return res.status(400).json({ success: false, error: 'Parameter parent wajib diisi.' });
    ...
    try {
        for (let g = 0; g < groupCount; g++) {
            ...
            for (let c = 0; c < combinations.length; c++) {
                ...
                try {
                    await mergeVideosCopyWithOptionalAudio([v1Path, v2Path], outputPath, soundPathToUse, { tempDir: path.join(__dirname, '_tmp_uploads') });
                    successCount++;
                }
                catch (err) {
                    failCount++;
        ...
        try {
            ...
            if (remainingVideos.length === 0) {
                ...
                for (const file of allFiles) {
                    try {
                        fs.unlinkSync(path.join(config.videoFolder, file));
                    }
                    catch { }
                }
...
app.get('/api/grok/rate-limits', (req, res) => {
    res.json(getGrokRateLimits());
});
app.post('/api/grok/clear-rate-limit', (req, res) => {
...
app.post('/api/grok/mark-downloaded', (req, res) => {
    ...
    if (!stateFile)
        return res.status(400).json({ error: 'Missing stateFile' });
    const list = Array.isArray(filenames) ? filenames : (filename ? [filename] : []);
    if (list.length === 0)
        return res.status(400).json({ error: 'Missing filename or filenames' });
    for (const f of list) {
...
app.post('/api/ytbot/config/save', (req, res) => {
    ...
    if (hashtags !== undefined)
        data.states[stateFile].hashtags = hashtags;
    if (scheduleDate !== undefined)
        data.states[stateFile].scheduleDate = scheduleDate;
...
app.post('/api/ytbot/links/add', (req, res) => {
    ...
    if (!stateFile || !link)
        return res.status(400).json({ error: 'stateFile dan link diperlukan' });
    const data = loadYtbotData();
    if (!data.states[stateFile]) {
...
// ── YTBOT ORCHESTRATION ──
async function ytbotRunState(stateFile) {
    if (!ytbotRunning)
        return;
    const data = loadYtbotData();
    const cfg = data.states[stateFile];
    ...
    while (ytbotRunning) {
        ...
        if (qIdx >= 0)
            ytbotQueue[qIdx] = qEntry;
        else
            ytbotQueue.push(qEntry);
        ytbotBroadcastQueue();
...
// ── FBBOT ORCHESTRATION ──
async function fbbotRunState(stateFile) {
    if (!fbbotRunning)
        return;
    ...
    while (fbbotRunning) {
        ...
        const qEntry = { stateName, stateFile, videoCount: batch.length, scheduleStart: `${schedDate} ${schedTime}`, scheduleEnd: endStr, active: true };
        if (qIdx >= 0)
            fbbotQueue[qIdx] = qEntry;
        else
            fbbotQueue.push(qEntry);
        fbbotBroadcastQueue();
...
app.post('/api/fb/config/save', (req, res) => {
    ...
    if (headless !== undefined)
        data.states[stateFile].headless = !!headless;
    saveFbbotData(data);
    res.json({ success: true });
});
            ...
            cookies: cookiesArray.map((c) => {
                ...
                if (c.sameSite) {
                    ...
                    if (lower === 'lax')
                        ...
                        sameSite = 'Strict';
                    else if (lower === 'none' || lower === 'no_restriction' || lower === 'unspecified')
                        sameSite = 'None';
                }
                return {
                    name: c.name,
...
function saveAutopullConfig(config) {
    try {
...
function resetGrokbotProgress(overrides = {}) {
    grokbotProgress = {
...
app.get('/api/grokbot/stock', (req, res) => {
    ...
    if (!stateFile || typeof stateFile !== 'string')
        return res.status(400).json({ error: 'stateFile atau state diperlukan' });
...
app.post('/api/grokbot/generate-utama', async (req, res) => {
    ...
    const cfg = data.states[stateFile];
    if (!cfg)
        return res.status(400).json({ error: 'Config state belum disimpan!' });
    if (!cfg.grokState)
        return res.status(400).json({ error: 'Grok state belum dipilih!' });
    const tiktokStateName = stateFile.replace('tiktok-state-', '').replace('.json', '');
    const stateDownloadDir = path.join(GROK_DOWNLOAD_DIR, tiktokStateName);
    if (!fs.existsSync(stateDownloadDir))
    ...
    runGrokGenerator(grokConfig, grokbotLog, __dirname).then(() => {
        clearInterval(poll);
...
app.post('/api/grokbot/generate-cadangan', async (req, res) => {
    ...
    const poll = setInterval(() => {
        ...
        if (activeCount > 0)
            overallGen += Math.round((activeProgSum / activeCount) / 60);
        grokbotProgress.generate = Math.min(99, overallGen);
        grokbotProgress.mergedCount = stats.saved;
...
app.post('/api/grokbot/schedule-only', async (req, res) => {
    ...
    const stateDownloadDir = path.join(GROK_DOWNLOAD_DIR, tiktokStateName);
    if (!fs.existsSync(stateDownloadDir))
        return res.status(400).json({ error: 'Folder download belum ada untuk state ini.' });
    const exts = ['.mp4', '.mov', '.avi', '.mkv', '.webm'];
    const marksFile = path.join(stateDownloadDir, '.uploaded.json');
    let marks = {};
    try {
...
async function grokbotRunState(stateFile, isFullAuto = false) {
    if (!grokbotRunning)
        return;
    const data = loadGrokbotData();
    const cfg = data.states[stateFile];
    ...
    try {
        while (grokbotRunning) {
            ...
            if (qIdx >= 0)
                grokbotQueue[qIdx] = qEntry;
            else
                grokbotQueue.push(qEntry);
            grokbotBroadcastQueue();
            grokbotProgress.generate = 100;
            grokbotProgress.merge = 100;
            grokbotProgress.upload = 0;
        ...
        if (qIdx2 >= 0) {
            grokbotQueue[qIdx2].active = false;
            grokbotBroadcastQueue();
        }
...
function getStateNextTrigger(sf) {
    const data = loadGrokbotData();
    const cfg = data.states[sf];
    const stateName = sf.replace('tiktok-state-', '').replace('.json', '');
    if (!cfg)
...
async function grokbotRunFullAuto(stateFiles) {
...
function getRateLimitExpiryDate(availableAt, detectedAt) {
    if (!availableAt)
        return null;
...
function parseAvailableAt(availableAt) {
    if (!availableAt)
        return 15 * 60 * 1000; // default 15 mins fallback
...
async function grokbotRunInfinite(stateFiles) {
...
app.get('/api/grokbot/logs', (req, res) => {
    ...
    req.on('close', () => {
        ...
        if (idx >= 0)
            grokbotSseClients.splice(idx, 1);
    });
});
// ═══════════════════════════════════════════════════════════
...
function getGrokV2AccountPool(autoSwitch, fixedState) {
    const validStates = getSavedStates('grok')
        .filter(item => item.expiry.status !== 'expired')
        .filter(item => {
        try {
            JSON.parse(fs.readFileSync(path.join(GROK_STATES_DIR, item.filename), 'utf-8'));
            return true;
        }
        catch {
            return false;
...
async function runGrokGeneratorV2(config, log) {
    ...
    for (let i = 0; i < totalRawToGenerate; i++) {
        ...
        try {
            ...
            if (err instanceof RateLimitError || err.name === 'RateLimitError') {
                ...
                if (!limitedAccounts.includes(limitedState))
                    limitedAccounts.push(limitedState);
    ...
    if (completedRaw >= totalRawToGenerate)
...
function countPendingVideos(folder, marksName, prefix) {
    ...
    let marks = {};
    try {
        marks = JSON.parse(fs.readFileSync(path.join(folder, marksName), 'utf-8'));
    }
    catch { }
    try {
...
async function waitWhileRunning(durationMs, isStillRunning) {
    const stepMs = 2000;
    let elapsed = 0;
...
function nextScheduleBaseline(items, cfg, fallbackDate, fallbackTime) {
    if (items.length === 0)
        return { scheduleDate: fallbackDate, scheduleTime: fallbackTime };
...
function validateInfiniteSchedulePayload(initialRunAt, states) {
    if (typeof initialRunAt !== 'string' || isNaN(new Date(initialRunAt).getTime()))
        return 'Initial time tidak valid';
    if (!Array.isArray(states) || states.length === 0)
        return 'Minimal satu state diperlukan';
    const seen = new Set();
    for (const item of states) {
        if (!item || typeof item.stateFile !== 'string' || !item.stateFile)
            return 'State file tidak valid';
        if (seen.has(item.stateFile))
            return `State duplikat: ${item.stateFile}`;
...
app.post('/api/grokbotv2/config/save', (req, res) => {
    ...
    if (grokState !== undefined)
        s.grokState = grokState;
    if (autoSwitchGrokState !== undefined)
        s.autoSwitchGrokState = !!autoSwitchGrokState;
    if (promptFile !== undefined)
        s.promptFile = promptFile;
    if (bahanFolder !== undefined)
        s.bahanFolder = bahanFolder;
...
app.post('/api/grokbotv2/generate-utama', async (req, res) => {
    ...
    if (!cfg.autoSwitchGrokState && !cfg.grokState)
        return res.status(400).json({ error: 'Grok State belum dipilih atau aktifkan Auto Switch!' });
    if (getGrokV2AccountPool(!!cfg.autoSwitchGrokState, cfg.grokState).length === 0)
        return res.status(400).json({ error: 'Tidak ada Grok State valid yang dapat digunakan!' });
    const tiktokStateName = stateFile.replace('tiktok-state-', '').replace('.json', '');
...
app.post('/api/grokbotv2/infinite-generate', (req, res) => {
    ...
    if (Array.isArray(stateOptions)) {
        for (const option of stateOptions) {
            if (!option || !stateFiles.includes(option.stateFile) || !startData.states[option.stateFile])
                continue;
            const autoSwitch = !!option.autoSwitchGrokState;
            const grokState = typeof option.grokState === 'string' ? option.grokState : '';
            if (!autoSwitch && !grokState)
                return res.status(400).json({ success: false, error: `Grok State diperlukan untuk ${option.stateFile}` });
            if (getGrokV2AccountPool(autoSwitch, grokState).length === 0)
                return res.status(400).json({ success: false, error: `Tidak ada Grok State valid untuk ${option.stateFile}` });
        ...
        try {
            while (infiniteGenV2Running) {
                ...
                for (const stateFile of stateFiles) {
                    const cfg = data.states[stateFile];
                    if (!cfg || !cfg.promptFile || (!cfg.autoSwitchGrokState && !cfg.grokState))
                        continue;
                    if (getGrokV2AccountPool(!!cfg.autoSwitchGrokState, cfg.grokState).length === 0)
                        continue;
                    const stateName = stateFile.replace('tiktok-state-', '').replace('.json', '');
...
function saveGrokbotV2InfiniteSchedule(config) {
    const data = loadGrokbotV2Data();
    data.infiniteSchedule = config;
    saveGrokbotV2Data(data);
}
async function runGrokbotV2InfiniteSchedule() {
    if (grokbotv2InfiniteScheduleRunning)
        return;
    const initial = loadGrokbotV2Data().infiniteSchedule;
    if (!initial?.active || initial.states.length === 0)
        return;
    ...
    try {
        while (grokbotv2InfiniteScheduleRunning) {
            ...
            if (!schedule?.active || schedule.states.length === 0)
                break;
            ...
            const marksFile = path.join(stateDir, '.downloaded.json');
            try {
                downloaded = JSON.parse(fs.readFileSync(marksFile, 'utf-8'));
            }
            catch { }
...
app.post('/api/grokbotv2/infinite-schedule', (req, res) => {
    if (grokbotv2InfiniteScheduleRunning || grokbotv2FullAutoRunning)
        return res.status(400).json({ success: false, error: 'Infinite Schedule atau Full Auto sedang aktif' });
    const validationError = validateInfiniteSchedulePayload(req.body.initialRunAt, req.body.states);
    if (validationError)
        return res.status(400).json({ success: false, error: validationError });
    const states = req.body.states.map((item, index) => ({ ...item, order: index }));
    const config = { active: true, initialRunAt: req.body.initialRunAt, currentIndex: 0, started: false, states, updatedAt: new Date().toISOString() };
...
app.post('/api/grokbotv2/stop', async (req, res) => {
    ...
    if (stopDataV2.infiniteSchedule) {
        stopDataV2.infiniteSchedule.active = false;
        saveGrokbotV2Data(stopDataV2);
    }
    resetGrokbotv2Progress();
    grokbotv2BroadcastProgress();
...
app.post('/api/leonardo/prompts/delete', (req, res) => {
    const { id } = req.body;
    if (!id)
        return res.status(400).json({ error: 'ID prompt diperlukan' });
    const data = loadLeonardoData();
...
app.post('/api/leonardo/bahan/upload', bahanUpload.array('images', 100), (req, res) => {
    const folderName = req.body.folderName;
    if (!folderName) {
...
app.post('/api/leonardo/bahan/delete', (req, res) => {
    const { name } = req.body;
    if (!name)
        return res.status(400).json({ error: 'Nama bahan diperlukan' });
    const leonardoBahanDir = path.join(__dirname, 'bahan', 'leonardo');
...
app.post('/api/vidabot/full-auto', async (req, res) => {
    if (vidabotFullAutoRunning || vidabotInfiniteScheduleRunning)
        return res.status(400).json({ error: 'Full Auto atau Infinite Schedule Vidabot sedang berjalan!' });
    const { stateFiles } = req.body;
    if (!stateFiles || !Array.isArray(stateFiles) || stateFiles.length === 0) {
        return res.status(400).json({ error: 'stateFiles diperlukan' });
    }
...
app.post('/api/vidabot/infinite-generate', async (req, res) => {
    if (vidaInfiniteGenRunning || vidabotRunning || vidabotFullAutoRunning)
        return res.status(400).json({ error: 'Vidabot sedang menjalankan proses lain!' });
    const { stateFiles } = req.body;
    if (!stateFiles || !Array.isArray(stateFiles) || stateFiles.length === 0) {
        return res.status(400).json({ error: 'stateFiles diperlukan' });
    }
...
function saveVidabotInfiniteSchedule(config) {
    const data = loadVidabotData();
    data.infiniteSchedule = config;
    saveVidabotData(data);
}
async function runVidabotInfiniteSchedule() {
    if (vidabotInfiniteScheduleRunning)
        return;
    ...
    try {
        while (vidabotInfiniteScheduleRunning) {
            ...
            const marksFile = path.join(stateDir, '.uploaded.json');
            try {
                uploaded = JSON.parse(fs.readFileSync(marksFile, 'utf-8'));
            }
            catch { }
...
app.post('/api/vidabot/infinite-schedule', (req, res) => {
    if (vidabotInfiniteScheduleRunning || vidabotFullAutoRunning)
        return res.status(400).json({ success: false, error: 'Infinite Schedule atau Full Auto sedang aktif' });
    const validationError = validateInfiniteSchedulePayload(req.body.initialRunAt, req.body.states);
    if (validationError)
        return res.status(400).json({ success: false, error: validationError });
...
app.post('/api/vidabot/stop', async (req, res) => {
    ...
    if (stopDataVida.infiniteSchedule) {
        stopDataVida.infiniteSchedule.active = false;
        saveVidabotData(stopDataVida);
    }
    resetVidabotProgress();
    vidabotBroadcastProgress();
...
app.listen(PORT, () => {
    initAutopull();
    startWAPolling();
    if (loadGrokbotV2Data().infiniteSchedule?.active)
        void runGrokbotV2InfiniteSchedule();
    if (loadVidabotData().infiniteSchedule?.active)
        void runVidabotInfiniteSchedule();
