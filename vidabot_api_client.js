// vidabot_api_client.js - Client untuk memanggil API Vidabot (Veo)
import fs from 'fs';
    ...
    while (Date.now() - startTime < TIMEOUT_MS) {
        ...
        try {
            ...
            if ([301, 302, 303, 307, 308].includes(checkRes.status) || location) {
                ...
                const downloadDir = path.join(process.cwd(), 'vidabot-downloads');
                if (!fs.existsSync(downloadDir)) {
                    fs.mkdirSync(downloadDir, { recursive: true });
                }

                const fname = `vidabot_${Date.now()}_${Math.floor(Math.random() * 1000)}.mp4`;
