...
// ─── DATABASE OPERATIONS ─────────────────────────────────
export function loadLeonardoData() {
    if (!fs.existsSync(DATA_FILE)) {
        const initialData = { accounts: [], prompts: [] };
...
export function saveLeonardoData(data) {
    fs.writeFileSync(DATA_FILE, JSON.stringify(data, null, 2));
}
// ─── JWT RESOLVER & UTILS ────────────────────────────────
function findJWTs(obj, found = []) {
...
export async function getFreshJWT(cookieString) {
    ...
    try {
        ...
        if (!content)
            throw new Error('Empty response body from Better-Auth session API');
        let sessionData;
        try {
...
// ─── GRAPHQL API CORE ───────────────────────────────────
export async function sendGraphQLRequest(token, payload) {
...
export async function fetchCreditBalance(token) {
    let userSub = '';
    try {
        const parts = token.split('.');
        if (parts.length === 3) {
  ...
  `;
    ...
    if (!details)
        throw new Error("Gagal mengambil informasi kredit dari data user_details.");
    const totalCredits = (details.subscriptionTokens || 0) +
        (details.paidTokens || 0) +
        (details.rolloverTokens || 0);
    return {
...
export async function uploadInitImage(token, filePath) {
...
export async function triggerKlingGenerate(token, options) {
    const parameters = {
        width: options.width,
...
export async function checkGenerationStatus(token, generationId) {
    const query = `
...
export async function fetchGenerationVideoUrl(token, generationId) {
    const query = `
  ...
  `;
    ...
    const gen = response.data?.generations?.[0];
    if (!gen)
        throw new Error(`Hasil generation untuk ID ${generationId} tidak ditemukan.`);
...
export async function downloadVideoToLocal(videoUrl, generationId) {
