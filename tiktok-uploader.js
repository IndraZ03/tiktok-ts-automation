            ...
            try {
                ...
                if (!checked) {
                    try {
                        ...
                        if (await tuxRadio.count() > 0) {
                            ...
                            if (checked)
                                log('✓ Radio produk dipilih (Metode 2: TUXRadioStandalone click)');
                        }
                    }
                    catch (err) {
                ...
                if (!checked) {
                    try {
                        ...
                        if (await productRow.count() > 0) {
                            ...
                            checked = await firstRadio.isChecked().catch(() => false);
                            if (checked)
                                log('✓ Radio produk dipilih (Metode 3: product-tb-row click)');
                        }
                    }
                    catch (err) {
                    ...
                    try {
                        ...
                        if (radioId) {
                            ...
                            if (await label.count() > 0) {
                                ...
                                checked = await firstRadio.isChecked().catch(() => false);
                                if (checked)
                                    log('✓ Radio produk dipilih (Metode 4: label for click)');
                            }
                        }
                    }
                    catch (err) {
                ...
                if (!checked) {
                    try {
                        ...
                        checked = await firstRadio.isChecked().catch(() => false);
                        if (checked)
                            log('✓ Radio produk dipilih (Metode 5: dispatchEvent click)');
                    }
                    catch (err) {
...
// ═══════════════════════════════════════════════════════════
export async function runUpload(config, log, onVideoUploaded) {
    isRunning = true;
    // ── Validate state file ──
